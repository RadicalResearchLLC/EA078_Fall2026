...
We also will import geospatial environmental data set for our test examples. We are going to use a curated version of the [CalEnviroscreen5.0](https://oehha.ca.gov/calenviroscreen/report/calenviroscreen-50) data. The data for the full state of California can be acquired as a zipped esri shapefile [.shp](https://data.ca.gov/dataset/calenviroscreen-5-0). 
...
Lastly, there is the pipe operator `|>`. It allows existing data or information to be passed to the next line of code after some operation has been done on the data. The pipe operator says, 'take the output from this line of code, and now do this next thing'. I like to think of them as Super Mario pipes that Mario enters and is transported to another place. For now, take it on faith that this is the most useful operator in the tidyverse and will be used throughout this course.
...
nc <- st_read(system.file("shape/nc.shp", package="sf")) |> #import data
  st_transform(crs = 4326) # transform the coordinate reference system to WGS84 
...
### Calenviroscreen5.0 geoJSON

The second dataset is hosted on the [github](https://github.com) repository where the class materials are version controlled. The code below names the internet URL where the data is stored as `URL.path`. If you go to the URL [link](), you will see the raw .geoJSON formatted spatial data file.
...
```{r}
#| label: fig-CalEJ
#| fig-cap: Leaflet map with Calenviroscreen 5.0
#| echo: false
...
pal2 <- colorQuantile(palette = 'YlGn', domain = nc$BIR79, n = 5)  ## This is a different color scale

leaflet() |> 
  addTiles() |> 
  addPolygons(data = nc, 
              stroke = FALSE,
              color = ~pal2(BIR79), ##we change it here
              fillOpacity = 0.8)
...
1.  Create a colorPalette for the SoCalEJ **dieselP** variable. Choose **one** of the following color Palettes from @fig-colorPalettes
2.  Create a map for the SoCalEJ dataset with census tracts color coded with your chosen color.
3.  Add a Legend to the map

That map might look like @fig-DPM but should hopefully have a different color palette.
...
palDPM <- colorNumeric(palette = 'Greys', domain = SoCalEJ$dieselP, n = 5)

leaflet(data = SoCalEJ) |> 
  addTiles() |> 
  setView(lat = 34, lng = -117.60, zoom = 9) |> 
  addPolygons(stroke = FALSE,
              color = ~palDPM(dieselP),
              fillOpacity = 0.8) |> 
