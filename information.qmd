...
![A gross image of particles emissions from a sneeze](images/COVID_mask.png){#fig-Disgusting}
...
This code imports monthly mean Mauna Loa CO~2~ data from [NOAA GMD CMDL](https://www.gml.noaa.gov/ccgg/trends/data.html), renames the columns, and then displays the bottom five rows to make sure it shows what we think it should. We'll show you how to do this next session.
...
The CO~2~ data is displayed in @fig-KeelingCurve1 and @fig-KeelingBase. How does the y-axis scale affect the interpretation of the same dataset?
...
plot1 <- co2 %>% 
  ...
  geom_line() +
  theme_bw() +
  labs( y = expression(Concentration~CO['2']~"(ppm)"),
        x = 'Year', 
      )
...
```{r}
#| label: fig-KeelingBase
#| fig-cap: The Keeling Curve showing monthly average CO~2~ concentrations (ppm) at Mauna Loa
#| warning: false

plot2 <- plot1 + 
  scale_y_continuous(limits = c(0,440)) +
  labs(y = 'Concentration CO2 (ppm)')
...
![Rcolorbrewer color palettes](https://datavizpyr.com/wp-content/uploads/2020/01/R_ColorBrewer_all_palettes.jpg){#fig-colorPalettes}
...
Looking for more on color palettes? Check out this [r-graph-gallery](https://r-graph-gallery.com/color-palette-finder) post, or this [color harmony](https://medium.com/global-maksimum-data-information-technologies/color-harmony-in-data-visualization-2610f9dab390) blog post for marketing design.
